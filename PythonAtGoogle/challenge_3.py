...
def recordPage (someUrl, someWebpage):
  for word in someWebpage.split():
    if not 'link' in word and not '<' in word:
...
# hashtable = {}
#  for word in someWebpage.split():
#    if word not in hashtable and not 'link' in word and not '<' in word:
#      hashtable[word] = 1
#      #print word#
#
#recordPage(url, result)
...
#  savelinks = set()
#  for link in links:
#    #if 'g.com' not in link:
#    #  savelinks.add(link)
#    #  savelinks += crawlForLinks(link)
#    links += crawlForLinks(link)
#  return set(links)
#
#allLinks = crawlForLinks(url)
...
highestCount = sortedKeys [-1]
lowestCount = sortedKeys [0]
bestUrl = info[highestCount]
worstUrl = info[lowestCount]
print('%s occurs the most times (%d) on %s' % (search, highestCount, bestUrl))
print('%s occurs the most times (%d) on %s' % (search, lowestCount, worstUrl))
